...
uint16_t current_cell_id;
uint16_t current_lac_id;
uint16_t current_index_round;
...
static unsigned int fbsb_count = 0;
...
uint64_t data_sample = 0;
uint64_t num_biterr = 0;
uint64_t num_bitcount = 0;
...
static int rx_ph_data_ind(struct osmocom_ms *ms, struct msgb *msg)
{
	...
	bcch_arfcn = ntohs(dl->band_arfcn);
	if (data_sample > 20){
		sample_offset = convert_fn_offset(bcch_arfcn, current_cell_id, current_lac_id, meas->last_fn, current_index_round);
		receiver_offset = sample_offset / (double) (bcch_arfcn > 512 ? 5000000 : 2000000) * 1625000.0/6.0*2;
		if (sample_offset > 0 && sample_offset != last_offset){
			insert_biterr_round(bcch_arfcn, current_cell_id, current_lac_id, (double)num_biterr/num_bitcount, receiver_offset, meas->last_fn, current_index_round);
			num_biterr = 0;
			num_bitcount = 0;
		...
		data_sample++;
		num_biterr = num_biterr + dl->num_biterr;
		num_bitcount = num_bitcount + 456;
	}
	...
	gsmtap_send(gsmtap_inst, bcch_arfcn, chan_ts,
...
static int rx_ph_burst_ind(struct osmocom_ms *ms, struct msgb *msg)
{
  ...
  struct rx_meas_stat *meas = &ms->meas;
  meas->last_fn = ntohl(bi->frame_nr);
...
//	      );
	...
	// dainv5 modified for hw reset
	if (fbsb_count>0) {
		fbsb_count--;
	}
	DEBUGP(DL1C, "rx_ph_burst_ind fbsb_count = %d\n", fbsb_count);
...
int l1ctl_tx_wait_kc_state(struct osmocom_ms *ms){
	struct msgb * msg;
	msg = osmo_l1_alloc(L1CTL_WAIT_KC);
	if (!msg)
		return -1;
	LOGP(DL1C, LOGL_NOTICE, "Sending wait_kc_state\n");
	return osmo_send_l1(ms, msg);
}

int l1ctl_tx_cell_lac(struct osmocom_ms *ms, uint16_t cell_id, uint16_t lac_id) {
	struct msgb * msg;
	struct l1ctl_cell_lac_info* cell_lac_info;

	msg = osmo_l1_alloc(L1CTL_CELL_LAC_INFO);
	if (!msg)
		return -1;

	cell_lac_info = (struct l1ctl_cell_lac_info*) msgb_put(msg, sizeof(*cell_lac_info));
	// LOGP(DL1C, LOGL_NOTICE, "L1ctl cell_id: %u, lac_id: %u\n", cell_id, lac_id);
	cell_lac_info->cell_id = cell_id;
	cell_lac_info->lac_id = lac_id;

	/* Update cell_id, lac_id for decode info database */
	current_cell_id = cell_id;
	current_lac_id = lac_id;
	return osmo_send_l1(ms, msg);
}

int l1ctl_tx_ccch_conf(struct osmocom_ms *ms, uint8_t ccch_conf) {
	struct msgb* msg;
	struct l1ctl_ccch_conf* ccch_conf_info;

	msg = osmo_l1_alloc(L1CTL_CCCH_CONF);
	if (!msg)
		return -1;

	ccch_conf_info = (struct ccch_conf_info*) msgb_put(msg, sizeof(*ccch_conf_info));
	ccch_conf_info->ccch_conf = ccch_conf;
	return osmo_send_l1(ms, msg);
}

int l1ctl_tx_arfcn(struct osmocom_ms *ms, uint16_t band_arfcn, uint64_t sample_start, uint8_t ccch_config, uint16_t cell_id, uint16_t lac_id, uint32_t index_round){
	struct msgb * msg;
	struct l1ctl_h0* h0;

	msg = osmo_l1_alloc(L1CTL_H0);
	if (!msg)
		return -1;

	LOGP(DL1C, LOGL_INFO, "Tx arfcn \n");
	current_index_round = index_round;

	h0 = (struct l1ctl_h0 *) msgb_put(msg, sizeof(*h0));
	h0 -> band_arfcn = band_arfcn;
	h0 -> sample_start = sample_start;
	h0 -> ccch_config = ccch_config;
	h0 -> cell_id = cell_id;
	h0 -> lac_id = lac_id;
	h0 -> index_round = index_round;

	return osmo_send_l1(ms, msg);
}

int l1ctl_tx_hopping_args(struct osmocom_ms *ms, uint16_t band_arfcn, uint16_t old_bcch_arfcn, uint8_t maio, uint8_t hsn, uint8_t tseq, uint8_t chan_nr, uint32_t frame_nr, uint8_t ch_ts, uint8_t ma_len, uint16_t* ma, uint16_t cell_id, uint16_t lac_id, uint32_t index_round){
    struct msgb * msg;
    struct l1ctl_info_ul *ul;
    struct l1ctl_hopping_args *hopa;
    int i, rc;
    
    
    msg = osmo_l1_alloc(L1CTL_HOPPING_ARGS);
    if (!msg)
        return -1;
    ...
    hopa = (struct l1ctl_hopping_args *) msgb_put(msg, sizeof(*hopa));
    hopa->band_arfcn = band_arfcn;
	hopa->old_bcch_arfcn = old_bcch_arfcn;
    hopa->h1.maio = maio;
    hopa->h1.hsn = hsn;
    hopa->h1.n = ma_len;
    hopa->tseq = tseq;
    hopa->frame_nr = frame_nr;
    hopa->ch_ts = ch_ts;
	hopa->cell_id = cell_id;
	hopa->lac_id = lac_id;
	hopa->index_round = index_round;
    
    for (i=0; i<ma_len; i++){
        hopa->h1.ma[i] = ma[i];
    }
    
    rc = osmo_send_l1(ms, msg);
    return 0;
}

/* Transmit L1CTL_DM_EST_REQ */
int l1ctl_tx_dm_est_req_h01(struct osmocom_ms *ms, uint16_t band_arfcn,
                           uint8_t chan_nr, uint8_t tsc, uint8_t tch_mode,
			   uint8_t audio_mode, unsigned int fn)
{
	struct msgb *msg;
	struct l1ctl_info_ul *ul;
	struct l1ctl_dm_est_req *req;
...
static int rx_l1_cell_lac_info(struct osmocom_ms *ms, struct msgb *msg){
	struct l1ctl_cell_lac_info *cell_lac_info;

	if (msgb_l1len(msg) < sizeof(*cell_lac_info)) {
		LOGP(DL1C, LOGL_ERROR, "CELL LAC INFO MSG too short "
			"(len=%u), missing measurement results\n", msgb_l1len(msg));
		return -1;
	}

	cell_lac_info = msg->l1h;
	osmo_signal_dispatch(SS_L1CTL, S_L1CTL_CELL_LAC_INFO, cell_lac_info);
	return 0;
}
...
int l1ctl_recv(struct osmocom_ms *ms, struct msgb *msg)
{
	...
	case L1CTL_TRAFFIC_CONF:
		msgb_free(msg);
		break;
	case L1CTL_CELL_LAC_INFO:
		rx_l1_cell_lac_info(ms, msg);
		break;
	default:
