...
static char *layer2_socket_path_default = "/tmp/osmocom_l2";
static char *layer2_socket_path = NULL;
...
static char * conf_path = "/etc/grd/config.ini";
uint8_t phoneid = 0;
...
static void build_config(char **opt, struct option **option)
{
	...
	app = l23_app_info();
	*opt = talloc_asprintf(l23_ctx, "hs:S:i:v:d:u:X:T:I:f:B:F:V:M:%s",
...
extern uint32_t d_h;
extern uint8_t d_target;
extern uint8_t d_target_list[41][9];
extern uint8_t d_blacklist_len;
extern uint8_t d_blacklist[41][9];
extern uint8_t d_tmsi_finder_flag;
extern uint8_t d_is_catch_voice;
extern uint8_t d_is_catch_sms;
extern int ccch_quit;
extern uint32_t d_index_round;
...
static void handle_options(int argc, char **argv)
{
	...
	while (1) {
		...
		case 'i':
			gsmtap_ip = optarg;
			fprintf(stderr, "gsmtap_ip in handle_options: %s\n", gsmtap_ip);
			break;
		case 'u':
			vty_ip = optarg;
			break;
		...
		case 'I':
			...
			break;
		case 'f':
			conf_path = talloc_strdup(l23_ctx, optarg);
			break;
		case 'T':
			d_h = atoi(optarg);
			break;
		case 'X':
			...
			token = strtok(optarg, ",");
			while (token != NULL) {
				memcpy(d_target_list[d_target], token, 8 * sizeof(char));
				d_target++;
				token = strtok(NULL, ",");
			}
			break;
		case 'B':
			...
			token = strtok(optarg, ",");
			while (token != NULL) {
				memcpy(d_blacklist[d_blacklist_len], token, 8 * sizeof(char));
				d_blacklist_len++;
				token = strtok(NULL, ",");
			}
			break;
		case 'F':
			d_tmsi_finder_flag = atoi(optarg);
			break;
		case 'V':
			d_is_catch_voice = atoi(optarg);
			break;
		case 'M':
			d_is_catch_sms = atoi(optarg);
			break;
...
int main(int argc, char **argv)
{
	...
	/* Load file config */
	memset(&sys, 0, sizeof(T_SYSCONF));
	LoadConfig(&sys, conf_path);

	if (phoneid >= 0 && phoneid < sys.total_worker) {
		sys.id = phoneid;
		// ms->test_arfcn = d_arfcn;
		if (sys.workers[sys.id].socket_path == NULL)
			layer2_socket_path = layer2_socket_path_default;
		else
			layer2_socket_path = sys.workers[sys.id].socket_path;
	} else {
