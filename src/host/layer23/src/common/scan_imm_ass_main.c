...
static void build_config(char **opt, struct option **option)
{
	...
	app = l23_app_info();
	*opt = talloc_asprintf(l23_ctx, "hs:S:a:i:v:d:u:A:P:o:I:f:C:L:J:%s",
...
extern uint32_t d_arfcn;
extern int d_father_pid;
extern uint64_t d_sample_start;
extern uint16_t d_cell_id;
extern uint16_t d_lac_id;
...
static void handle_options(int argc, char **argv)
{
	...
	while (1) {
		...
		case 'A':
			...
			break;
		case 'P':
			d_father_pid = atoi(optarg);
			break;
		case 'o':
			d_sample_start = atoll(optarg);
			printf("Sample start: %llu\n", d_sample_start);
			break;
		case 'C':
			d_cell_id = atoi(optarg);
			break;
		case 'L':
			d_lac_id = atoi(optarg);
			break;
		case 'J':
			d_index_round = atoi(optarg);
			break;
