...
static void build_config(char **opt, struct option **option)
{
	...
	app = l23_app_info();
	*opt = talloc_asprintf(l23_ctx, "hs:S:a:i:v:d:u:T:I:f:M:K:%s",
...
extern uint32_t d_h;
extern uint32_t id_rebuild_jump_event;
extern int ccch_quit;
extern uint8_t is_offline_mode;
...
static void handle_options(int argc, char **argv)
{
	...
	while (1) {
		...
		case 'M':
			...
			break;
		case 'K':
			id_rebuild_jump_event = atoi(optarg);
			break;
