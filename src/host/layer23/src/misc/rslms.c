...
static int rslms_rx_udata_ind(struct msgb *msg, struct osmocom_ms *ms)
{
	...
	if (rllh->chan_nr == RSL_CHAN_PCH_AGCH || rllh->chan_nr == 0x92 || rllh->chan_nr == 0x94 || rllh->chan_nr == 0x96) {
		rc = gsm48_rx_ccch(msg, ms);
	} else if (rllh->chan_nr == RSL_CHAN_BCCH || rllh->chan_nr == 0x82 || rllh->chan_nr == 0x84 || rllh->chan_nr == 0x86) {
