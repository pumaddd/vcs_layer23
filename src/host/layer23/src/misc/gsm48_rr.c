...
static int gsm48_rr_rx_cip_mode_cmd(struct osmocom_ms *ms, struct msgb *msg)
{
	...
	if (rr->cipher_on)
		l1ctl_tx_crypto_req(ms, rr->cd_now.chan_nr, rr->cipher_type + 1, subscr->key, 8);
	else
		l1ctl_tx_crypto_req(ms, rr->cd_now.chan_nr, 0, NULL, 0);
	...
	uint16_t *ma, uint8_t *ma_len)
{
	...
	if (cd->mob_alloc_lv[0]) {
		struct gsm_sysinfo_freq *freq = ss.freq;
...
int gsm48_rr_rx_ass_cmd_parser(struct osmocom_ms *ms, char * msg, int msg_length, uint16_t * ma, uint8_t * ma_len, uint8_t * voicemode, T_MULTIRATE_CONFIG* multirate_conf)
{
	//struct gsm48_rrlayer *rr = &ms->rrlayer;
	//struct gsm322_cellsel *cs = &ms->cellsel;
	//struct gsm48_sysinfo *s = cs->si;
	struct gsm48_hdr *gh = msg;
	struct gsm48_ass_cmd *ac = (struct gsm48_ass_cmd *)gh->data;
	int payload_len = msg_length - sizeof(*gh) - sizeof(*ac);
	struct tlv_parsed tp;
	struct gsm48_rr_cd *cda;// &rr->cd_after;
	struct gsm48_rr_cd *cdb;// &rr->cd_before;
	uint8_t ch_type, ch_subch, ch_ts;
...
// 	uint8_t ma_len;
	...
	cda = malloc(sizeof(struct gsm48_rr_cd));
	if (cda == NULL)
		return -1;
	cdb = malloc(sizeof(struct gsm48_rr_cd));
	if (cdb == NULL)
	{
		free(cda);
		return -1;
...
// 			sizeof(cda->cell_desc_lv));
	...
	if (TLVP_PRESENT(&tp, GSM48_IE_CHANMODE_1)) {
		...
		*voicemode = *TLVP_VAL(&tp, GSM48_IE_CHANMODE_1);
		LOGP(DRR, LOGL_INFO, " both: changing channel mode 0x%02x\n",
			*voicemode);
	}
	else
	{
		voicemode = 0;
		//cda->mode = cdb->mode = 0;//rr->cd_now.mode;
	}

	// dainv5 modified for speech rate
	memset(multirate_conf, 0, sizeof(T_MULTIRATE_CONFIG));
	if (TLVP_PRESENT(&tp, GSM48_IE_MUL_RATE_CFG)) {
		uint8_t *val = TLVP_VAL(&tp, GSM48_IE_MUL_RATE_CFG);
		multirate_conf->length =  TLVP_LEN(&tp, GSM48_IE_MUL_RATE_CFG);
		memcpy(multirate_conf->data, val, multirate_conf->length);

		LOGP(DRR, LOGL_INFO, " multirate config length 0x%02x\n", multirate_conf->length);
	}
	...
	if (TLVP_PRESENT(&tp, GSM48_IE_CIP_MODE_SET)) {
		...
		cda->cipher = cdb->cipher = 0;// rr->cd_now.cipher;
	...
	gsm48_decode_power_cmd_acc(
		...
		&cda->ind_tx_power, NULL);
	cdb->ind_tx_power = cda->ind_tx_power;
	cda->ind_ta = cdb->ind_ta = 0;// rr->cd_now.ind_ta; /* same cell */
	LOGP(DRR, LOGL_INFO, " both: (tx_power %d TA %d)\n", cda->ind_tx_power,
		cda->ind_ta);
	...
	if (before_time) {
		...
		if (cause){
			free(cda);
			free(cdb);
			// dainv5 modified for ass_cmd parser
			// return gsm48_rr_tx_ass_fail(ms, cause, RSL_MT_DATA_REQ);
			return cause;
			// dainv5 end of modifed
		}
	}
	cause = gsm48_rr_render_ma(ms, cda, ma, ma_len);
	if (cause){
...
int gsm48_cc_rx_setup_parser(char * msg, uint8_t msg_length, char * phonenum, uint16_t phone_len)
{
	struct gsm48_hdr *gh = msg;
	unsigned int payload_len = msg_length - sizeof(*gh);
	struct tlv_parsed tp;
...
// 			TLVP_VAL(&tp, GSM48_IE_SIGNAL) - 1);
	...
	if (TLVP_PRESENT(&tp, GSM48_IE_CALLING_BCD)) {
		...
		memcpy(phonenum, setup.calling.number, phone_len);
	}
	...
	if (TLVP_PRESENT(&tp, GSM48_IE_CALLED_BCD)) {
		...
		memcpy(phonenum, setup.called.number, phone_len);
	}
...
// 		memcpy(phonenum, setup.redirecting.number, phone_len);
	}
	/* user-user */
...
int gsm48_rr_rx_hando_cmd_parser(struct osmocom_ms *ms, char * msg, int msg_length, uint16_t * arfcn, uint16_t *ma, uint8_t * ma_len, struct gsm48_chan_desc *chan_desc, uint8_t * voice_mode)
{
	struct gsm48_rrlayer *rr = &ms->rrlayer;
	struct gsm322_cellsel *cs = &ms->cellsel;
	struct gsm48_sysinfo *s = cs->si;
	struct gsm48_hdr *gh = (void*)msg;
	struct gsm48_ho_cmd *ho = (struct gsm48_ho_cmd *)gh->data;
	int payload_len = msg_length - sizeof(*gh);
	struct tlv_parsed tp;
	struct gsm48_rr_cd *cda = &rr->cd_after;
	struct gsm48_rr_cd *cdb = &rr->cd_before;
	//uint16_t arfcn;
	...
	if (TLVP_PRESENT(&tp, GSM48_IE_CH_DESC_1_BEFORE)) {
		...
		if (ccd->h0.h) {
			...
			gsm48_decode_chan_h0(ccd, &cdb->tsc, &cdb->arfcn);
			if (s == NULL) {
				LOGP(DRR, LOGL_INFO, "Si in cell select is NULL\n");
			} else if (gsm_refer_pcs(cs->arfcn, s))
				cdb->arfcn |= ARFCN_PCS;
			LOGP(DRR, LOGL_INFO, " before: (chan_nr 0x%02x "
				"ARFCN %s TS %u SS %u TSC %u)\n", ccd->chan_nr,
				gsm_print_arfcn(cdb->arfcn),
	...
	memset(chan_desc, 0, sizeof(struct gsm48_chan_desc));
	memcpy(chan_desc, &ho->chan_desc, sizeof(struct gsm48_chan_desc));
	/* starting time */
#ifdef TEST_STARTING_TIMER
	cda->start = 1;
	cda->start_tm.fn = (ms->meas.last_fn + TEST_STARTING_TIMER) % 42432;
	...
	if (before_time) {
		cause = gsm48_rr_render_ma(ms, cdb, ma, ma_len);
		if (cause)
			return cause; // gsm48_rr_tx_hando_fail(ms, cause, RSL_MT_DATA_REQ);
	}
	cause = gsm48_rr_render_ma(ms, cda, ma, ma_len);
	if (cause)
		return cause; //gsm48_rr_tx_hando_fail(ms, cause, RSL_MT_DATA_REQ);
