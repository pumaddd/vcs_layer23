...
int trx_if_cmd_set_arfcn(struct trx_instance *trx, uint16_t arfcn, uint64_t sample_start, uint16_t cell_id, uint16_t lac_id, uint32_t index_round){
    return trx_ctrl_cmd(trx, 1, "SETARFCN", "%u %llu %u %u %u", arfcn, sample_start, cell_id, lac_id, index_round);
}

int trx_if_send_wait_kc(struct trx_instance *trx, uint8_t state){
	LOGP(DTRX, LOGL_INFO, "trx_if send wait kc \n");
	return trx_ctrl_cmd(trx, 1, "SETWAITKC", "%u", state);
}

int trx_if_send_cell_lac(struct trx_instance *trx, uint16_t cell_id, uint16_t lac_id) {
	LOGP(DTRX, LOGL_INFO, "In trx_if_send_cell_lac \n");
	return trx_ctrl_cmd(trx, 1, "SETCELLLAC", "%u %u", cell_id, lac_id);
}
...
int trx_if_cmd_set_hopping_args(struct trx_instance *trx, uint16_t arfcn, uint16_t old_bcch_arfcn,
	...
	*(ptr - 1) = '\0';
    return trx_ctrl_cmd(trx, 8, "SETHOPARGS", "%u %u %u %u %u %u %u %u %u %u %s", arfcn, old_bcch_arfcn, maio, hsn , tseq, frame_nr, cell_id, lac_id, time_slot, index_round, ma_buf);
}
...
static int trx_ctrl_read_cb(struct osmo_fd *ofd, unsigned int what)
{
	...
	else if(!strncmp(tcm->cmd + 4, "SETHOPARGS", 4))
	{
		char *str = buf + strlen("SETHOPARGS") + 7;
		LOGP(DTRX, LOGL_DEBUG, "Receive cell id, lac id: '%s'\n", str);
		char *token;
		char count = 1;
		uint16_t cell_id, lac_id;
		int len = 0;
			token = strtok(str, " ");
			while (token != NULL) {
				...
				default:
					LOGP(DTRX, LOGL_ERROR, "Receive more args via SETHOPARGS RSP\n");
					break;
				}
				token = strtok(NULL, " ");
				count++;
			}
		LOGP(DTRX, LOGL_NOTICE, "Receive cell id: %u, lac id: %u\n", cell_id, lac_id);
		l1ctl_tx_cell_lac_info(trx -> l1l, cell_id, lac_id);
	}
...
static int trx_data_rx_cb(struct osmo_fd *ofd, unsigned int what)
{
	...
	arfcn = buf[9] << 8 | buf[10];
	trx->band_arfcn = arfcn;
	ul = !!(arfcn & 0x4000);
	rssi = -(int8_t) buf[5];
	toa256 = ((int16_t) (buf[6] << 8) | buf[7]);
	snr = buf[8];
	/* Copy and convert bits {254..0} to sbits {-127..127} */
	osmo_ubit2sbit(bits, buf + 12, 148);
	...
	/* Poke scheduler */
	sched_trx_handle_rx_burst(trx, tn, fn, bits, 148, rssi, toa256, snr, ul);
