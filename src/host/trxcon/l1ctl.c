...
int l1ctl_tx_cell_lac_info(struct l1ctl_link *l1l, uint16_t cell_id, uint16_t lac_id){
	struct l1ctl_cell_lac_info *cell_lac_info;
	struct msgb *msg;

	msg = l1ctl_alloc_msg(L1CTL_CELL_LAC_INFO);
	if (msg == NULL)
		return -ENOMEM;
...
int l1ctl_tx_burst_ind(struct l1ctl_link *l1l, struct l1ctl_burst_ind *data)
{
	struct msgb *msg;
    
	msg = l1ctl_alloc_msg(L1CTL_BURST_IND);
	if (msg == NULL)
		return -ENOMEM;
    
    size_t len = sizeof(struct l1ctl_burst_ind);
    struct l1ctl_burst_ind *dl = (struct l1ctl_burst_ind *) msgb_put(msg, len);
    memcpy(dl, data, len);
...
static int l1ctl_rx_h0(struct l1ctl_link *l1l, struct msgb *msg){
	struct l1ctl_h0 *h0;

	trx_if_cmd_poweroff(l1l->trx);
	h0 = (struct l1ctl_h0 *) msg->l1h;
	LOGP(DL1C, LOGL_NOTICE, "Received band_arfcn: %d, index round: %lu, sample_start: %llu, ccch_config: %d, cell_id: %u, lac_id: %u\n", h0->band_arfcn, h0->index_round, h0->sample_start, h0->ccch_config, h0->cell_id, h0->lac_id);
	trx_if_cmd_set_arfcn(l1l->trx, h0->band_arfcn, h0->sample_start, h0->cell_id, h0->lac_id, h0->index_round);

	/* Config time slot rev data */
	sched_trx_reset(l1l->trx, false);
	switch (h0->ccch_config)
	{
	case 0:
	...
	case 2:
		sched_trx_configure_ts(l1l->trx, 0, GSM_PCHAN_CCCH);
		sched_trx_configure_ts(l1l->trx, 2, GSM_PCHAN_CCCH);
		break;
	case 4:
		sched_trx_configure_ts(l1l->trx, 0, GSM_PCHAN_CCCH);
		sched_trx_configure_ts(l1l->trx, 2, GSM_PCHAN_CCCH);
		sched_trx_configure_ts(l1l->trx, 4, GSM_PCHAN_CCCH);
		break;
	case 6:
		sched_trx_configure_ts(l1l->trx, 0, GSM_PCHAN_CCCH);
		sched_trx_configure_ts(l1l->trx, 2, GSM_PCHAN_CCCH);
		sched_trx_configure_ts(l1l->trx, 4, GSM_PCHAN_CCCH);
		sched_trx_configure_ts(l1l->trx, 6, GSM_PCHAN_CCCH);
		break;
	default:
		LOGP(DL1C, LOGL_NOTICE, "Not handle ccch_config %d\n", h0->ccch_config);
		return 0;
		break;
	}
...
static int l1ctl_rx_wait_kc(struct l1ctl_link *l1l, struct msgb *msg){
	LOGP(DL1C, LOGL_NOTICE, "Received wait_kc\n");
	trx_if_send_wait_kc(l1l->trx, 1);
	return 0;
}

static int l1ctl_rx_cell_lac(struct l1ctl_link *l1l, struct msgb *msg) {
	struct l1ctl_cell_lac_info* cell_lac_info;

	cell_lac_info = (struct l1ctl_info_ul *) msg->l1h;

	LOGP(DL1C, LOGL_NOTICE, "Received request set cell_id: %u, lac_id: %u to layer1\n", cell_lac_info->cell_id, cell_lac_info->lac_id);
	trx_if_send_cell_lac(l1l->trx, cell_lac_info->cell_id, cell_lac_info->lac_id);
	return 0;
}

static int l1ctl_tx_ccch_conf(struct l1ctl_link *l1l, struct msgb *msg) {
	struct l1ctl_ccch_conf* ccch_conf_info;

	ccch_conf_info = (struct l1ctl_ccch_conf *) msg->l1h;
	LOGP(DL1C, LOGL_NOTICE, "Received reconfig ccch-conf: %u\n", ccch_conf_info->ccch_conf);

	switch (ccch_conf_info->ccch_conf)
	{
	case 0:
		sched_trx_configure_ts(l1l->trx, 0, GSM_PCHAN_CCCH);
		break;
...
static int l1ctl_rx_hopping_args(struct l1ctl_link *l1l, struct msgb *msg){
    struct l1ctl_info_ul *ul;
    struct l1ctl_hopping_args *hopa;
    
    LOGP(DL1C, LOGL_NOTICE, "Received hopping args\n");
    
    ul = (struct l1ctl_info_ul *) msg->l1h;
    hopa = (struct l1ctl_hopping_args *) ul->payload;
    
    trx_if_cmd_poweroff(l1l->trx);
    sched_trx_reset(l1l->trx, false);
    trx_if_cmd_set_hopping_args(l1l->trx, hopa->band_arfcn, hopa->old_bcch_arfcn, hopa->h1.maio, hopa->h1.hsn, hopa->tseq, hopa->frame_nr, hopa->h1.n, hopa->h1.ma, hopa->cell_id, hopa->lac_id, hopa->ch_ts, hopa->index_round);
    
    LOGP(DL1C, LOGL_NOTICE, "frame_nr: %d, band_arfcn: %d, index_round: %lu, maio: %d, hsn: %d, tseq: %d, ch_ts: %d, cell_id: %u, lac_id: %u\n", hopa->frame_nr, hopa->band_arfcn, hopa->index_round, hopa->h1.maio, hopa->h1.hsn, hopa->tseq, hopa->ch_ts, hopa->cell_id, hopa->lac_id);
    return 0;
}
...
int l1ctl_rx_cb(struct l1ctl_link *l1l, struct msgb *msg)
{
	...
	case L1CTL_HOPPING_ARGS:
		return l1ctl_rx_hopping_args(l1l, msg);
	case L1CTL_H0:
		return l1ctl_rx_h0(l1l, msg);
	case L1CTL_WAIT_KC:
		return l1ctl_rx_wait_kc(l1l, msg);
	case L1CTL_CELL_LAC_INFO:
		return l1ctl_rx_cell_lac(l1l, msg);
	case L1CTL_CCCH_CONF:
		return l1ctl_tx_ccch_conf(l1l, msg);
