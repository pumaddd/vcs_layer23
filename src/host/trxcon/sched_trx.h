...
typedef int trx_lchan_rx_func(struct trx_instance *trx,
	...
	int8_t rssi, int16_t toa256, uint8_t snr);
...
struct trx_ts {
	...
	uint32_t mf_last_downlink_fn;
	uint32_t mf_last_uplink_fn;
...
int sched_send_dt_ind(struct trx_instance *trx, struct trx_ts *ts,
	...
	int bit_error_count, bool dec_failed, bool traffic, uint8_t snr);
